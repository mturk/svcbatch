...
/**
 * Set when pipeio thread ends
 */
static HANDLE    pipesignaled     = 0;
...
static DWORD killprocesstree(DWORD pid, UINT err)
{
    ...
    do {
        if (e.th32ParentProcessID == pid) {
            if (c == TBUFSIZ) {
                /**
                 * Child has more then 512 subprocesses ???
                 */
                svcsyserror(__LINE__, ERROR_OUTOFMEMORY, L"killprocesstree");
                break;
            }
    ...
    for (i = 0; i < c; i++) {
        /**
         * Close each child and its children
         *
         * TODO: Verify if the child has the same
         *       parent as we do, or the child pid
         *       was recycled.
         */
#if defined(_DBGVIEW)
...
static int getourexenames(void)
{
    wchar_t    *buf = 0;
    wchar_t    *exe;
    DWORD       siz = TBUFSIZ;
    ...
    exe = getrealpathname(buf, 0);
    xfree(buf);
    if (exe != 0) {
        int i = (int)xwcslen(exe);
        while (--i > 0) {
            if (exe[i] == L'\\') {
                exe[i] = L'\0';
                servicebase = exe;
 ...
 */
static DWORD reportsvcstatus(DWORD cs, DWORD wh)
{
    ...
    else if (cs == SERVICE_STOPPED) {
        if (wh != 0)
            servicestatus.dwServiceSpecificExitCode = wh;
...
static PSECURITY_ATTRIBUTES getnullsa(void)
{
    static SECURITY_ATTRIBUTES sa;
    PSECURITY_DESCRIPTOR sd;

    sd = (PSECURITY_DESCRIPTOR)xmalloc(SECURITY_DESCRIPTOR_MIN_LENGTH);
...
static DWORD WINAPI svcstopthread(LPVOID unused)
{
...
#if defined(_DBGVIEW)
        dbgprintf(__FUNCTION__, "   killing: %d", cmdexeproc.dwProcessId);
#endif
    ...
    if (GetExitCodeProcess(cmdexeproc.hProcess, &rc) == 0)
        svcsyserror(__LINE__, GetLastError(), L"GetExitCodeProcess");
...
static DWORD WINAPI svcpipethread(LPVOID unused)
{
    ...
    for (;;) {
        ...
        if ((ReadFile(redirectedpiperd, rb, HBUFSIZ, &rd, 0) == 0) || (rd == 0)) {
             ...
             * ERROR_BROKEN_PIPE means that child process
             * has terminated and closed its side of the pipe.
             */
            rc = GetLastError();
            break;
        }
    ...
    if (rc ==  ERROR_BROKEN_PIPE)
        dbgprintf(__FUNCTION__, "   done rv ERROR_BROKEN_PIPE");
    else
        dbgprintf(__FUNCTION__, "   done rv %d", rc);
#endif

    SetEvent(pipesignaled);
    XENDTHREAD(0);
}
...
static DWORD WINAPI svcmonitorthread(LPVOID unused)
{
    ...
    while (WaitForSingleObject(monitorevent, INFINITE) == WAIT_OBJECT_0) {
        ...
        if (cc == 0) {
            ...
            dbgprintf(__FUNCTION__, "unknown CTRL: %d", cc);
 ...
 * This thread is created by SCM. It should provided
 * service name as first argument.
 */
void WINAPI servicemain(DWORD argc, wchar_t **argv)
{
    ...
    if ((rv = opensvclog(1)) != 0) {
        rv = svcsyserror(__LINE__, rv, L"OpenLog");
        goto finished;
    }
    logconfig();
    logfflush();
 ...
 * atexit callbacks
 */
static void svcfreeconsole(void)
{
    FreeConsole();
}

static void rmconsolehandler(void)
{
...
int wmain(int argc, const wchar_t **wargv, const wchar_t **wenv)
{
    ...
    if (IS_INVALID_HANDLE(serviceended) ||
        ...
        IS_INVALID_HANDLE(pipesignaled) ||
    ...
    SAFE_CLOSE_HANDLE(pipesignaled);
