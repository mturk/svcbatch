...
static LPCWSTR cmdoptions  = L"cef:hik:m:nopr:st:w";
    ...
    while ((opt = xwgetopt(argc, argv, cmdoptions)) != EOF) {
        switch (opt) {
            case ':':
                ...
                else if ((xwoptarg[0] == L'-') &&
                         (xwoptarg[1] == L'-') && (xwoptarg[2] == WNUL)) {
                    /**
                     * Next arguments will be for stop
                     */
                    sa = 1;
                    if (svcstop == NULL)
