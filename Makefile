...
!ENDIF
!IF DEFINED(_SVCBATCH_LITE) && "$(_SVCBATCH_LITE)" == ""
!ERROR _SVCBATCH_LITE variable cannot be empty. Use _SVCBATCH_LITE=1, _SVCBATCH_LITE=2. etc
!ENDIF
...
CFLAGS = $(CFLAGS) -D_SVCBATCH_LITE=$(_SVCBATCH_LITE)
RFLAGS = $(RFLAGS) /d _SVCBATCH_LITE=$(_SVCBATCH_LITE)
!ENDIF

OBJECTS = $(WORKDIR)\$(PROJECT).obj
